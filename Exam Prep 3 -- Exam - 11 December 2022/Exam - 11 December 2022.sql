...
/****        03. Update           ***************************************************************************************************************************************/
/* There was a mistake at the Armenian airport with the airplanes. You must change the airplanes for all flights that 
departure from Armenia. Raise the airplane_id by 1 for all flights flying from Armenia. */
UPDATE flights
...
SELECT 
    CONCAT(UPPER(LEFT(passengers.last_name, 2)), passengers.country_id) AS flight_code,
    CONCAT(first_name, ' ', last_name) AS full_name,
    country_id
FROM passengers
LEFT JOIN flights_passengers ON passengers.id = flights_passengers.passenger_id
WHERE flights_passengers.passenger_id IS NULL
...
FROM flights_passengers
INNER JOIN flights ON flights_passengers.flight_id = flights.id
INNER JOIN countries ON flights.destination_country = countries.id
...
FROM flights
INNER JOIN countries ON flights.destination_country = countries.id
INNER JOIN flights_passengers ON flights.id = flights_passengers.flight_id
GROUP BY countries.name, countries.currency
HAVING COUNT(flights_passengers.passenger_id) >= 20
ORDER BY booked_tickets DESC;
...
•	flight_code
•	departure
•	day_part (Morning is from 5AM to 11:59AM, Afternoon is from 12PM to 4:59PM, 
Evening is from 5PM to 8:59PM and Night is from 9PM to 4:59AM) 											*/ 
...
/* Create a user defined function with the name udf_count_flights_from_country(country VARCHAR(50)) that receives a country name and returns the total number of flights departing from that country.
Required Columns
...
BEGIN
    RETURN (
        SELECT COUNT(*)
        FROM flights
        WHERE departure_country = (SELECT id FROM countries WHERE name = country)
    );
END //
DELIMITER ;
...
/****		11. Delay flight			***************************************************************************************************************************************/
/* Your task is to find the flight and change the delay status to true and the departure time by 30 minutes.
Create a stored procedure udp_delay_flight which accepts the following parameters:
•	code VARCHAR(50) 														*/

DELIMITER //
CREATE PROCEDURE udp_delay_flight(IN flight_code VARCHAR(50))
BEGIN
    UPDATE flights
    SET has_delay = TRUE,
        departure = departure + INTERVAL 30 MINUTE
